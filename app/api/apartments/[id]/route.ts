...
export async function GET(
  ...
  try {
    ...
    const transformedApartment = {
      ...
      price: apartment.price, // Оставляем числом для календаря
      ...
      cleaningTime: apartment.cleaningTime,
      hostRating: 4.5,
    };
...
// PATCH - обновить объявление (с отправкой на повторную модерацию) ИЛИ настройки времени
export async function PATCH(
  ...
  try {
    ...
    // Если это JSON-запрос (для обновления времени)
    if (contentType.includes("application/json")) {
      const body = await request.json();

      const updateData: any = {};

      // Обработка времени заезда/выезда
      ...
      if (body.cleaningTime) updateData.cleaningTime = body.cleaningTime;

      // Проверка валидности времени
      if (body.checkInTime && body.checkOutTime) {
        const [inHours, inMinutes] = body.checkInTime.split(":").map(Number);
        const [outHours, outMinutes] = body.checkOutTime.split(":").map(Number);
        ...
        if (inTotal <= outTotal) {
          return NextResponse.json(
            { error: "Время заезда должно быть после времени выезда" },
            { status: 400 }
          );
        }
      }
    ...
    // Если это FormData (для обновления всей информации)
